...
open class TextEditorView: UIView {
    ...
    public var textCountInset: UIEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0) {
        didSet {
            setNeedsUpdateConstraints()
        }
    }
    ...
    public var placeholderFont = UIFont.theme.bodyLarge {
        didSet {
    ...
    public var minHeight: CGFloat = 20
    ...
    /// 是否影藏字数统计， 默认false.
    public var isHiddenTextCountLabel: Bool = true {
        didSet {
            setNeedsUpdateConstraints()
        }
    }
