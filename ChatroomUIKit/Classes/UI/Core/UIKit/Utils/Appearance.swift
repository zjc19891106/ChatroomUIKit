...
@objcMembers final public class Appearance: NSObject {
    ...
    static var `default`: Appearance = Appearance()
    ...
    var pageContainerTitleBarItemWidth = 114
    
    var pageContainerConstraintsSize = CGSizeMake(ScreenWidth, ScreenHeight*(3/5.0))
    
    var giftContainerConstraintsSize = CGSizeMake(ScreenWidth, ScreenHeight/2.0)
    
    /// Description You can change the overall cell layout style of the barrage area by setting the current properties.
    var barrageCellStyle: ChatBarrageCellStyle = .all
    ...
    var colorHues: [CGFloat] = [Appearance.default.primaryHue,Appearance.default.secondaryHue,Appearance.default.errorHue,Appearance.default.neutralHue,Appearance.default.neutralSpecialHue,Appearance.default.gradientEndHue]
    ...
    var primaryHue: CGFloat = 203/360.0
    ...
    var secondaryHue: CGFloat = 155/360.0
    ...
    var errorHue: CGFloat = 350/360.0
    
    /// Description You can change the hue of the base color, and then change the thirteen UIColor objects of the related color series. The UI components that use the relevant color series in the chat room UIKit will also change accordingly. The default value is 203/360.0.
    var neutralHue: CGFloat = 203/360.0
    ...
    var emojiMap: Dictionary<String,UIImage> = [:]
    
    /// Description language code mirror type.
    var targetLanguage: LanguageType = .English
    
    /// Description ActionSheet data source.
    var defaultMessageActions: [ActionSheetItemProtocol] = [ActionSheetItem(title: "Translate".chatroom.localize, type: .normal),ActionSheetItem(title: "Delete", type: .normal),ActionSheetItem(title: "Mute", type: .normal),ActionSheetItem(title: "unmute", type: .normal),ActionSheetItem(title: "Report", type: .destructive)]
    
    /// Description ActionSheet row height.
    var actionSheetRowHeight: CGFloat = 56
}
