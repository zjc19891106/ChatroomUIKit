...
final class UIComponentsExampleViewController: UIViewController {
    ...
    lazy var giftBarrages: GiftMessageList = {
        GiftMessageList(frame: CGRect(x: 10, y: 400, width: self.view.frame.width-100, height: Appearance.giftAreaRowHeight*2),source:nil)
    }()
    
    lazy var barrageList: MessageList = {
        MessageList(frame: CGRect(x: 0, y: 500, width: self.view.frame.width-50, height: 200))
    }()
    ...
    lazy var inputBar: MessageInputBar = {
        MessageInputBar(frame: CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: 52),text: nil,placeHolder: "Input words.")
    }()
